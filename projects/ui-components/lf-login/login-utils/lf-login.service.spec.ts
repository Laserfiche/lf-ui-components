...
describe('LfLoginService', () => {
  ...
  beforeEach(() => {
    ...
    const loginProvider = jasmine.createSpyObj('LoginProvider', ['getTokenClient']);
    loginProvider.getTokenClient.and.returnValue(new TokenClient('test-url'));
  ...
  it('exchangeCodeForTokenAsync should logout if error', () => {
    ...
    expect(service.logoutCompletedInService.emit).toHaveBeenCalled();
