...
export function LFTimePickerPlugin(): Plugin {
	return function (fp: Instance) {
        ...
        try {
          ...
          */
          locale = locale ? locale : fp.config.locale;
					return window["flatpickr"].parseDate(date, dateFormat, false, locale);
				} catch (e) {
					return new Date();
				}
			}
        ...
        try {
          locale = locale ? locale : fp.config.locale;
					// if (isCustomFormat(fp.config.dateFormat)) {
            const time = window["flatpickr"].formatDate(date, dateTimeService.fromDisplayDateTimeFormatToFlatpickrFormat(format, true), locale);
    ...
    // This may be specific to forms-layout // Revise
		fp.config.onValueUpdate.push(function () {
			if (!fp.calendarContainer.classList.contains("open")) {
				...
				fp.input.value = window["flatpickr"].formatDate(parsedDate, fp.config.dateFormat, fp.config.locale);
...
function parseTimeFromPicker(fp: Instance): string {
  let value = fp.hourElement.value + ":" + fp.minuteElement.value;
