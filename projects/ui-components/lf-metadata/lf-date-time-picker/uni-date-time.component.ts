// Core and essentials
import { Component, ViewChild, ElementRef, OnInit, AfterViewInit, HostBinding, Input, AfterContentInit, EventEmitter, Output } from "@angular/core";
...
import { StateData, UniState, StateDataDateTime, StateDataTime, UniControlType, UniComponentSettings, UniComponentConfig, FormChangeEvent, FormChangeSource, FormatType } from "./uni-date-time.common";
...
export class UniDateTimeComponent implements OnInit, AfterViewInit, AfterContentInit {
    ...
    @Output() onBlurEvent: EventEmitter<{ state: UniState, source: FormChangeSource, event: FormChangeEvent, component: UniDateTimeComponent }> = new EventEmitter();
    @Output() onDateIconClickEvent: EventEmitter<{ state: UniState, event: FormChangeEvent, component: UniDateTimeComponent }> = new EventEmitter();
    @Output() onTimeIconClickEvent: EventEmitter<{ state: UniState, event: FormChangeEvent, component: UniDateTimeComponent }> = new EventEmitter();
    ...
    ngAfterViewInit() {
        if (!this.config.isDisplayOnly && !this.state.readonly) {
            ...
            if (this.settings.useCurrentDate) {
                ...
                defaultDateToSet = parsedDefaultDate ? this.dateTimeService.format({
            ...
            if (this.settings.useCurrentTime) {
                if (!this.config.isDisplayOnly) {
                    // format using current language
                    defaultTimeToSet = this.dateTimeService.format({ dateTimeObj: new Date(), timeFormat: this.settings.timeFormat, language: this.config.language });
                }
            } else if (this.settings.defaultTimeOfDate) {
                // format using current language
                defaultTimeToSet = this.dateTimeService.format({
                    // Parse using defaultDate language
                    dateTimeObj: this.dateTimeService.parse({ timeStr: this.settings.defaultTimeOfDate, timeFormat: this.defaultTimeFormat, language: this.config.defaultDateLanguage }),
            ...
            if (!this.settings.showTimeOnly && this.dateDiv.nativeElement) {
                this.date = flatpickr(this.dateDiv.nativeElement, {
                    ...
                    defaultDate: defaultDate + (this.settings.combinedDateTime && defaultDate != "today" && defaultTime ? (" " + defaultTime) : ""),
    ...
    private populateSettings() {
        ...
        this.settings.capturedInBackend = (this.settings.useCurrentDate || this.settings.useCurrentTime ) && this.useBackendDateTimeForCurrentDateTime &&
            !this.state.disableUseCurrentDateTime && (this.settings.readOnly || this.state.readonly);
    }
    ...
    private autoCorrectSettings() {
        ...
        this.settings.capturedInBackend = (this.settings.useCurrentDate || this.settings.useCurrentTime) && this.useBackendDateTimeForCurrentDateTime &&
            !this.state.disableUseCurrentDateTime && ((this.config.isDisplayOnly && this.settings.readOnly) || this.state.readonly);
    ...
    private populateConfig() {
        ...
        this.useBackendDateTimeForCurrentDateTime = this.config.useBackendDateTimeForCurrentDateTime ? this.config.useBackendDateTimeForCurrentDateTime : false;
    }

    public populateStrings() {
        this.strings = (!this.strings || this.config.silent) ? this.dateTimeService.getDefaultStrings(this.config.silent) : { ...this.dateTimeService.getDefaultStrings(this.config.silent), ...this.strings };
        this.strings = (!this.customStrings || this.config.silent ) ? this.strings : { ...this.strings, ...this.customStrings };
    }
    ...
    substituteTokens(value: string, keepIfAccepted: boolean): string {
        ...
        value = isToken && keepIfAccepted ?
            (this.settings.acceptTokens ? value : this.dateTimeService.removeTokens(value))   // keep token string only if we accept tokens
            : isToken && !keepIfAccepted ? this.dateTimeService.removeTokens(value)  // remove tokens if not accepted
    ...
    onBlurWithKey(key: string) {
        setTimeout(() => {
            ...
            this.onBlurEvent.emit({ state: this.state, source: FormChangeSource.User, event: key === "date" ? FormChangeEvent.DateBlur : FormChangeEvent.TimeBlur, component: this });
        });
    }
    ...
    processDateTimeValues(value: string | Date | undefined | null, isDate: boolean) {
        ...
        if (showTime) {
            if (typeof v === "string") {
                ...
                // format using current language
                dateStr = isDate ? this.dateTimeService.format({ dateTimeObj: v, dateFormat, language: this.config.language }) : oldDateStr;
                timeStr = (isDate && !combinedDateTime) ? oldTimeStr : this.dateTimeService.format({ dateTimeObj: v, timeFormat, language: this.config.language });
            }
            ...
            result = {
                dateTimeObj,
                dateStr: formatDateStr ? this.dateTimeService.format({ dateTimeObj, dateFormat, language: this.config.language }) : (this.dateTimeService.isToken(dateStr) ? dateStr : ""),
                timeStr: formatTimeStr ? this.dateTimeService.format({ dateTimeObj, timeFormat, language: this.config.language }) : (this.dateTimeService.isToken(timeStr) ? timeStr : "")
            };
        } else {
            // isDate must be true
            if (typeof v === "string") {
                // parse using current language
                dateTimeObj = v && !this.dateTimeService.isToken(dateStr) ? this.dateTimeService.parse({ dateStr: v, dateFormat, language: this.config.language }) : null;
            } else {
                dateTimeObj = v;
            }
            dateStr = dateStr? this.substituteTokens(dateStr, true) : "";
    ...
    onDateTimeChange(value: string | Date | undefined | null, isDate: boolean, event: FormChangeEvent) {
        ...
        const controlType = this.settings.combinedDateTime ? UniControlType.DateTime :
                isDate ? UniControlType.DateTime_date :
                this.settings.showTimeOnly ? UniControlType.Time : UniControlType.DateTime_time;
    ...
    assignSubAbstractControl() {
        ...
        this.dateTimeControlName = this.dateTimeControlName ? this.dateTimeControlName : `${this.id}-${UniControlType.DateTime}`;
    ...
    setDate(newDate: string | Date) {
        if (newDate) {
            let dateStr = '';
            let timeStr = this.state.data ? this.state.data.timeStr : '';

            let dateTimeObj = (typeof newDate == 'string') ? this.dateTimeService.createDateTimeObject(newDate, this.config.defaultDateFormat) : newDate;
            if (typeof newDate == 'string') {
                dateStr = newDate;
            } else {
                const dataFromDate = this.dateTimeService.generateDateTimeData('*', this.config.defaultDateFormat, newDate);
    ...
    //--
    getErrorTypeWithHighestPriority(abstractControl: AbstractControl): string {  // Revise
        ...
        if (abstractControl.errors) {
            ...
            if (errorTypes.length > 1) {
                for (let orderIndex = 0; orderIndex < errorsPriority.length; orderIndex++) {
                    let errorType = errorsPriority[orderIndex].errorType;
                    let mappedErrorType = errorsPriority[orderIndex].mappedErrorType;
