...
- `[lf-field-container]`, `[lf-field-adhoc-container]`, `[lf-field-template-container]`:
  - Fixed multi-value Date/Time/DateTime fields adding a duplicate row with a copied value instead of one blank row after committing a value.
  - Fixed Tab focusing a Date/Time/DateTime field from opening its picker, which could trap keyboard focus in an infinite loop of new rows for multi-value Time fields. Tab now moves to the next field; Enter or a click opens the picker.
  - Fixed a Number field with a min/max constraint clearing the entered value while typing, which could prevent the field from ever becoming valid even after correcting the input.
  - Fixed numeric field validation treating `0` as an empty value and skipping constraint checks.
...
- Fixed the issue when the locale contains language only.
...
- `[AppLocalizationService]`: Do not log warning every time a window message event is triggered

### Chore & Maintenance

- Updated dependency with vulnerabilities (`@babel/traverse@7.22.10 -> @babel/traverse@7.23.2`)
